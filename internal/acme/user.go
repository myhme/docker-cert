...
func NewUser(email, keyPath string, logger Logger) (*User, error) {
	if email == "" {
		return nil, fmt.Errorf("ACME user email cannot be empty")
	}
	if keyPath == "" {
	...
	if _, err := os.Stat(keyPath); err == nil {
		...
		user.logger.Printf("Loaded existing ACME user private key from %s", keyPath)
		...
		user.logger.Printf("Generated and saved new ACME user private key to %s", keyPath)
	} else {
